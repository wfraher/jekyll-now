...
To have a machine make decisions in an environment, it needs to weigh decisions by valuing different inputs, or parts of the state. This is done with a set of parameters, which we denote $\theta$.Each $a_t, s_t$, and $r_t$ denotes the action, state, and reward at each time step of the environment. The time step is denoted $t$. The way that an agent computes an action, given a set of parameters, is called a policy. A policy with parameters $\theta$ is denoted $\pi_\theta$. This policy is actually a function, that maps states to actions. So at any timestep $t$, $a_t$ = $\pi_\theta(s_t)$. This just means that a policy with parameters $\theta$ maps a state at timestep $t$ to the action at timestep $t$.
